// Dense magical floating emojis all over the screen!
function createFlake() {
    const types = ['❄️', '🎁', '🎄', '✨', '⭐', '🎅']; // Snow, gifts, trees, sparkles, stars, Santa!
    ...
    flake.style.animationDuration = ${duration}s, ${duration * 3}s; // FIXED with backticks!
...
setInterval(createFlake, 80); // Super dense coverage!

// Instrumental music setup
const music = document.getElementById('music');
if (music) {
    music.volume = 0.3;
    music.play().catch(() => {}); // Try autoplay (may need tap on mobile)
}

// Surprise effect on tap
function surprise() {
    // Massive colorful confetti
...
// Reliable tap/click (desktop + mobile)
